...
from votes.stv import DeterministicElection, Election, ElectionError
...
def read_blt(file_path: Path, det_picks: dict[int, int]):
    ...
    with file_path.open() as fp:
        ...
        while True:
            ...
            votes.extend(v)
            line = fp.readline().strip()
        e = DeterministicElection(
            set(range(1, count + 1)), votes, seats, random_picks=det_picks
        )
        if withdrawn:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {2: 6, 3: 10, 4: 11},
            [4, 7, 8, 13],
            {2: [6, 10, 11, 12], 3: [10, 11, 12], 4: [11, 12]},
        ),
        (
            "SCw.blt",
            {2: 6, 3: 10, 4: 11},
...
def test_blts(data, file, winners, tiebreak, picks):
    ...
    for i in e.actlog:
        if i["type"] == "tiebreak":
            ...
            assert sorted([int(c) for c in d["candidates"]]) == sorted(
                tiebreak[d["round"]]
            )
