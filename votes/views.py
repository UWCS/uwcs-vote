...
class AdminView(PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        ctxt["closed_elections"] = (
            self.get_queryset()
            .filter(
                Q(stvvote__isnull=False)
                | Q(aprvvote__isnull=False)
                | Q(fptpvote__isnull=False),
                open=False,
            )
            .distinct()
        )
...
class DateTicketView(PermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        sumembers = SUMember.objects.filter(
            firstSeen__lte=form.cleaned_data["bought_membership_before"]
        )
...
class ApprovalVoteView(UserPassesTestMixin, TemplateView):
    ...
    def test_func(self):
        ...
        return (
            self.request.user.member.ticket_set.filter(
                election=self.election, spent=False
            ).exists()
            and (
                su := SUMember.objects.filter(
                    uniqueId=self.request.user.member.uni_id
                ).first()
            )
            and set(self.election.required_webgroups or []).issubset(set(su.webgroups))
        )
    ...
    def post(self, request, **kwargs):
        ...
        if errors:
            ...
            return HttpResponseRedirect(
                reverse(
                    "votes:vote_done",
                    kwargs={"election": self.election.id, "slug": vote.uuid},
                )
            )
...
class FPTPVoteView(UserPassesTestMixin, TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        if errors:
            ...
            vote = FPTPVote(
                uuid=ticket.uuid, election=self.election, selection_id=selection
            )
            vote.save()
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            withdrawn = set(
                map(
                    attrgetter("id"),
                    self.election.candidate_set.filter(state=Candidate.State.WITHDRAWN),
                )
            )
            votes = []
