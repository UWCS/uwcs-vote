...
def vote_breakdown_table(election: Election):
    ...
    for i in actions:
        ...
        if type_ == "round" and not shortcircuit and detail["round"] != 0:
            ...
            total.append(sum_)
            elected = sum(
                [1 for x in detail["candidates"].values() if x["status"] == "ELECTED"]
            )
            if elected == election.seats and any(
                detail["candidates"][candidate]["status"] == "HOPEFUL"
                for candidate in candidates
            ):
                shortcircuit = True
                sum_ = detail["wastage"]
        ...
        for i in range(rounds):
            ...
            if tiebreak[i] is not False:
                if key == tiebreak[i][0]:
                    ...
                    row.append(
                        (mark_safe('<i class="fas fa-circle"></i>'), 1, "standard")
                    )
                else:
